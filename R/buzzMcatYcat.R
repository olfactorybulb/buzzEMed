#' Fit a Bayesian Mediation Model (Categorical M & Y)
#'
...
#'
#' @param direct.coef.init Initial values for precisions and direct effects.
...
#'
#' @examples
#' # 1. Create data: Both M and Y are Binary
#' set.seed(101)
#' n <- 100
#' toy_data <- data.frame(
#'   predictor = rbinom(n, 1, 0.5),
#'   mediator1 = rbinom(n, 1, 0.3),
#'   mediator2 = rbinom(n, 1, 0.7),
#'   outcome   = rbinom(n, 1, 0.5)
#' )
#'
#' # 2. Fit the model
#' results <- buzzMcatYcat(
#'   dataset  = toy_data,
#'   X        = "predictor",

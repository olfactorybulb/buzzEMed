#' Fit a Bayesian Mediation Model (Binary M & Continuous Y)
#'
#' Fits a Bayesian mediation model specifically designed for binary mediators
...
#'
#' @param y.prec.init,direct.coef.init Initial values for precisions and direct effects.
...
#'
#' @examples
#' # 1. Create data: Binary M, Continuous Y
#' set.seed(789)
#' n <- 100
#' toy_data <- data.frame(
#'   predictor = rnorm(n),
#'   mediator1 = rbinom(n, 1, 0.4), # Binary
#'   mediator2 = rbinom(n, 1, 0.6), # Binary
#'   outcome   = rnorm(n)           # Continuous
#' )
#'
#' # 2. Fit the model
#' results <- buzzMcatYcont(
#'   dataset  = toy_data,
#'   X        = "predictor",
